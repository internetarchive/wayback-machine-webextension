function appendToParent (id, item, text_before, parent, show_item, text_after) {
  if (item) {
    ...
    if (show_item) {
      idTag.textContent = text_before + item + text_after;
      parent.appendChild(idTag);
    } else {
      idTag.textContent = text_before + text_after;
      parent.appendChild(idTag);
    }
  }
}
function url_getter () {
  ...
  $.getJSON(domaintools_api, function (data) {
    const parent = document.getElementById('show_domaintools_data');
    if (data.response.results_count !== 0) {
      ...
      const whoisTag = document.getElementById('whois');
      whoisTag.setAttribute('href', data.response.results[0].whois_url);
    } else {
